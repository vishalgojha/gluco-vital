...
Deno.serve(async (req) => {
  try {
    ...
    const existingAchievements = await base44.asServiceRole.entities.UserAchievements.list('-created_date', 100);
    ...
    let result;
    if (userAchievements) {
      result = await base44.asServiceRole.entities.UserAchievements.update(
        userAchievements.id, 
        achievementData
      );
    } else {
      result = await base44.asServiceRole.entities.UserAchievements.create({
        ...achievementData,
