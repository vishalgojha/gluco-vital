...
Deno.serve(async (req) => {
  try {
    ...
    const scheduledReports = await base44.asServiceRole.entities.ScheduledReport.filter({ is_active: true });
    ...
    for (const schedule of scheduledReports) {
      ...
      if (shouldSend) {
        try {
          ...
          await base44.asServiceRole.entities.ScheduledReport.update(schedule.id, {
