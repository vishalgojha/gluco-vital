...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const profiles = await base44.asServiceRole.entities.PatientProfile.filter({
    ...
    for (const profile of profiles || []) {
      ...
      if (profile.whatsapp_medication_reminders) {
        try {
          medicationReminders = await base44.asServiceRole.entities.MedicationReminder.filter({
      ...
      if (profile.whatsapp_appointment_reminders) {
        try {
          ...
          const appointments = await base44.asServiceRole.entities.DoctorVisit.filter({
            user_email: profile.user_email,
            status: 'scheduled'
          });
