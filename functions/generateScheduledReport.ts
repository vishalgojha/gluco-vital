...
Deno.serve(async (req) => {
  try {
    ...
    const [logs, profile, reminders, adherenceLogs] = await Promise.all([
      base44.asServiceRole.entities.HealthLog.filter({ user_email: targetEmail }, '-created_date', 500),
      base44.asServiceRole.entities.PatientProfile.filter({ user_email: targetEmail }),
      base44.asServiceRole.entities.MedicationReminder.filter({ user_email: targetEmail, is_active: true }),
      base44.asServiceRole.entities.MedicationAdherence.filter({ user_email: targetEmail }, '-scheduled_time', 200)
    ]);
...
Provide insights in JSON format: {"insights": ["insight1", "insight2", "insight3"]}`;
    ...
    const savedReport = await base44.asServiceRole.entities.HealthReport.create({
    ...
    if (send_email) {
      ...
      const scheduleSettings = await base44.asServiceRole.entities.ScheduledReport.filter({ user_email: targetEmail });
      ...
      // Update last_sent_at
      if (settings) {
        await base44.asServiceRole.entities.ScheduledReport.update(settings.id, {
