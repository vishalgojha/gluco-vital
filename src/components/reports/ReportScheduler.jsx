...
export default function ReportScheduler({ user }) {
  ...
  const { data: schedule, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const results = await base44.entities.ScheduledReport.filter({ user_email: user?.email });
  ...
  const { data: coachConnections = [] } = useQuery({
    ...
    queryFn: () => base44.entities.CoachConnection.filter({ client_email: user?.email, status: 'active' }),
    enabled: !!user?.email
  });
