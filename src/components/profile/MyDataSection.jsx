  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      const [healthLogs, medicationReminders, labResults, labReports, doctorVisits, achievements] = await Promise.all([
        base44.entities.HealthLog.filter({ user_email: user.email }).catch(() => []),
        base44.entities.MedicationReminder.filter({ user_email: user.email }).catch(() => []),
        base44.entities.LabResult.filter({ user_email: user.email }).catch(() => []),
        base44.entities.LabReport.filter({ user_email: user.email }).catch(() => []),
        base44.entities.DoctorVisit.filter({ user_email: user.email }).catch(() => []),
        base44.entities.UserAchievements.filter({ user_email: user.email }).catch(() => [])
      ]);
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      for (const entityName of entitiesToDelete) {
        try {
          const records = await base44.entities[entityName].filter({ user_email: user.email });
