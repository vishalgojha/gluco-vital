...
export default function DoctorVisitTracker({ userEmail }) {
  ...
  const { data: visits = [] } = useQuery({
    ...
    queryFn: () => base44.entities.DoctorVisit.filter({ user_email: userEmail }, '-visit_date'),
    enabled: !!userEmail
  });

  const saveMutation = useMutation({
    mutationFn: (data) => base44.entities.DoctorVisit.create({ ...data, user_email: userEmail, status: 'scheduled' }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.DoctorVisit.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['doctor-visits'] });
