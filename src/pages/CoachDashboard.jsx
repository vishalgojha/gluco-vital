...
export default function CoachDashboard() {
  ...
  const inviteMutation = useMutation({
    mutationFn: async (data) => {
      const response = await base44.functions.invoke('inviteClient', data);
  ...
  const { data: connections = [], isLoading: connectionsLoading } = useQuery({
    ...
    queryFn: () => base44.entities.CoachConnection.filter({ coach_email: user?.email }),
    enabled: !!user?.email
  });
  ...
  const acceptMutation = useMutation({
    mutationFn: (id) => base44.entities.CoachConnection.update(id, { 
      status: "active",
  ...
  const declineMutation = useMutation({
    mutationFn: (id) => base44.entities.CoachConnection.update(id, { status: "revoked" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['coach-clients'] });
...
function ClientCard({ connection }) {
  ...
  const { data: profile } = useQuery({
    ...
    queryFn: () => base44.entities.PatientProfile.filter({ user_email: connection.client_email }),
    select: data => data?.[0]
  });
