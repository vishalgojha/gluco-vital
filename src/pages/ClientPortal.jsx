...
export default function ClientPortal() {
  ...
  const { data: connections = [], isLoading: connectionsLoading } = useQuery({
    ...
    queryFn: () => base44.entities.CoachConnection.filter({ coach_email: user?.email, status: "active" }),
    enabled: !!user?.email
  });
  ...
  const { data: clientProfile } = useQuery({
    ...
    queryFn: () => base44.entities.PatientProfile.filter({ user_email: selectedClient }),
    select: data => data?.[0],
  ...
  const { data: healthLogs = [] } = useQuery({
    ...
    queryFn: () => base44.entities.HealthLog.filter({ user_email: selectedClient }, '-created_date', 100),
    enabled: !!selectedClient
  });

  const { data: documents = [] } = useQuery({
    queryKey: ['client-documents', selectedClient],
    queryFn: () => base44.entities.HealthDocument.filter({ owner_email: selectedClient }, '-created_date', 20),
    enabled: !!selectedClient
  });

  const { data: achievements } = useQuery({
    queryKey: ['client-achievements', selectedClient],
    queryFn: () => base44.entities.UserAchievements.filter({ user_email: selectedClient }),
    select: data => data?.[0],
    enabled: !!selectedClient
  });
