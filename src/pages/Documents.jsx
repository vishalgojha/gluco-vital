...
export default function Documents() {
  ...
  const { data: documents = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.HealthDocument.filter({}, '-created_date', 200),
    enabled: !!user?.email
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.HealthDocument.delete(id),
    onSuccess: () => {
  ...
  const togglePinMutation = useMutation({
    mutationFn: (doc) => base44.entities.HealthDocument.update(doc.id, { is_pinned: !doc.is_pinned }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['health-documents'] });
    }
  });
