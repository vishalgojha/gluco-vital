...
export default function PatientDetail() {
  ...
  const { data: connection } = useQuery({
    ...
    queryFn: async () => {
      const conns = await base44.entities.DoctorConnection.filter({ id: connectionId });
      return conns[0];
    },
  ...
  const { data: profile } = useQuery({
    ...
    queryFn: () => base44.entities.PatientProfile.filter({ user_email: patientEmail }),
  ...
  const { data: reports = [] } = useQuery({
    ...
    queryFn: () => base44.entities.HealthReport.filter(
      { user_email: patientEmail },
      '-created_date',
      10
    ),
  ...
  const { data: feedback = [] } = useQuery({
    ...
    queryFn: () => base44.entities.DoctorFeedback.filter({ patient_email: patientEmail }),
    enabled: !!patientEmail
  });
