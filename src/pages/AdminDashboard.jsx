...
export default function AdminDashboard() {
  ...
  const { data: profiles = [] } = useQuery({
    ...
    queryFn: () => base44.entities.PatientProfile.list('-created_date', 500),
    enabled: user?.role === 'admin'
  });
  ...
  const { data: recentLogs = [] } = useQuery({
    queryKey: ['admin-logs'],
    queryFn: () => base44.entities.HealthLog.list('-created_date', 1000),
    enabled: user?.role === 'admin'
  });
  ...
  const { data: memories = [] } = useQuery({
    ...
    queryFn: () => base44.entities.ConversationMemory.list('-created_date', 500),
    enabled: user?.role === 'admin'
  });
